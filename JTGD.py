...
####派单数据入postgresql库,参数a为要入的数，b入的表名，c入的列名
...
def getProblemCellKpi(fpath):
    try:
        ...
        df_VOLTEJTD.loc[:, 'vccquestiontype'] = 'VoLTE无线接通差小区(语音)-20'
        df_VOLTEJTD.loc[:, 'vcequestiontype'] = 'VOLTEJTD'
        df_VOLTEJTD.loc[:, '无线接通率(QCI1)'] = df_VOLTEJTD['无线接通率(QCI1)'].apply(lambda x: format(x, '.2%'))
        df_VOLTEJTD.loc[:, 'vcproblemtarget1'] = "[无线接通率(QCI1):]" + df_VOLTEJTD['无线接通率(QCI1)'] + " [VoLTE话务量]:" + \
        ...
        df_ESRVCCQHC.loc[:, 'vccquestiontype'] = 'eSRVCC切换差小区-20'
        df_ESRVCCQHC.loc[:, 'vcequestiontype'] = 'ESRVCCQHC'
        df_ESRVCCQHC.loc[:, 'SRVCC无线切换成功率'] = df_ESRVCCQHC['SRVCC无线切换成功率'].apply(lambda x: format(x, '.2%'))
        df_ESRVCCQHC.loc[:, 'vcproblemtarget1'] = "[eSRVCC切换成功率]" + df_ESRVCCQHC['SRVCC无线切换成功率'] + "[eSRVCC切换失败次数]" \
        ...
        df_VOLTERABDXG.loc[:, 'vccquestiontype'] = 'VoLTE E-RAB掉线高小区(语音)-20'
        df_VOLTERABDXG.loc[:, 'vcequestiontype'] = 'VOLTERABDXG'
        df_VOLTERABDXG.loc[:, 'E-RAB掉线率(QCI1)(小区级)'] = df_VOLTERABDXG['E-RAB掉线率(QCI1)(小区级)'].apply(
            lambda x: format(x, '.2%'))
        df_VOLTERABDXG.loc[:, 'vcproblemtarget1'] = "[E-RAB掉线率(QCI1)(小区级)]:" + df_VOLTERABDXG['E-RAB掉线率(QCI1)(小区级)'] + \
        ...
        df_VOLTESXGDB.loc[:, 'vccquestiontype'] = 'VoLTE上行高丢包小区-20'
        df_VOLTESXGDB.loc[:, 'vcequestiontype'] = 'VOLTESXGDB'
        df_VOLTESXGDB.loc[:, '上行平均丢包率'] = df_VOLTESXGDB['上行平均丢包率'].apply(lambda x: format(x, '.2%'))
        df_VOLTESXGDB.loc[:, 'vcproblemtarget1'] = "[上行丢包率]:" + df_VOLTESXGDB['上行平均丢包率'] + "[上行丢包率采样点点数]:" + \
        ...
        df_VOLTEXXGDB.loc[:, 'vccquestiontype'] = 'VoLTE下行高丢包小区-20'
        df_VOLTEXXGDB.loc[:, 'vcequestiontype'] = 'VOLTEXXGDB'
        df_VOLTEXXGDB.loc[:, '下行平均丢包率'] = df_VOLTEXXGDB['下行平均丢包率'].apply(lambda x: format(x, '.2%'))
        df_VOLTEXXGDB.loc[:, 'vcproblemtarget1'] = "[下行丢包率]:" + df_VOLTEXXGDB['下行平均丢包率'] + "[下行丢包率采样点点数]:" + \
        ...
        df_VOLTESXGTZ.loc[:, 'vccquestiontype'] = 'VoLTE上行高吞字小区-20'
        df_VOLTESXGTZ.loc[:, 'vcequestiontype'] = 'VOLTESXGTZ'
        df_VOLTESXGTZ.loc[:, '上行高吞字采样点占比'] = df_VOLTESXGTZ['上行高吞字采样点占比'].apply(lambda x: format(x, '.2%'))
        df_VOLTESXGTZ.loc[:, 'vcproblemtarget1'] = "[上行高吞字率]:" + df_VOLTESXGTZ['上行高吞字采样点占比'] + "[上行丢包率采样点点数]:" + \
        ...
        df_VOLTEXXGTZ.loc[:, 'vccquestiontype'] = 'VoLTE下行高吞字小区-20'
        df_VOLTEXXGTZ.loc[:, 'vcequestiontype'] = 'VOLTEXXGTZ'
        df_VOLTEXXGTZ.loc[:, '下行高吞字采样点占比'] = df_VOLTEXXGTZ['下行高吞字采样点占比'].apply(lambda x: format(x, '.2%'))
        df_VOLTEXXGTZ.loc[:, 'vcproblemtarget1'] = "[上行高吞字率]:" + df_VOLTEXXGTZ['下行高吞字采样点占比'] + "[下行丢包率采样点点数]:" + \
                                                   df_VOLTEXXGTZ['下行总采样点数'].map(str)
        df_VOLTEXXGTZ = df_VOLTEXXGTZ[
            ['vccquestiontype', 'vcequestiontype', 'CGI', 'vcproblemtarget1', 'vcauxiliarypointer4']]
        df = df_VOLTEJTD.append(df_ESRVCCQHC).append(df_VOLTERABDXG).append(df_VOLTESXGDB).append(df_VOLTEXXGDB). \
...
def modifyGDFormat(df):
    df.loc[:, 'vcquestioncategory'] = "小区问题点"
    df.loc[:, 'vcnetworksystem'] = "LTE"
    df.loc[:, 'vcdatasource'] = "端到端信令分析优化"
    df.loc[:, 'vccgi'] = "460-00-" + df['CGI'].map(str)
    df.loc[:, 'vcspecialtag'] = "VOLTE两高两低-集团"
    df.loc[:, 'vcproblemtarget3'] = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time()))
    df.loc[:, 'vcproblemtarget5'] = "广东"
    df.loc[:, 'vcproblemtarget2'] = "VOLTE-JTGD" + '-' + (
...
if __name__ == "__main__":
    ...
    df.loc[:, 'starttime'] = np.where(df['vcequestiontype'].isin(['VOLTEJTD', 'ESRVCCQHC', 'VOLTERABDXG']), bb, dd)
    df.loc[:, 'endtime'] = np.where((df['vcequestiontype'].isin(['VOLTEJTD', 'ESRVCCQHC', 'VOLTERABDXG'])), aa, cc)
    ##筛选现网有业务工单
    df, df_all = chooseState(df, eptable)
    df = df.reset_index(drop=False)
    df.loc[:, 'vcauxiliarypointer6'] = "JT" + (df.index + 1 + suoyin).map(str)
    ...
    JTPDno = '集团下发派单数' + str(df_all.iloc[:, 0].size)
    write_csv(JTPDno)
    print(JTPDno)
    Notrue = '实际可派工单数' + str(send.iloc[:, 0].size)
    write_csv(Notrue)
    print(Notrue)
    ...
    ##派单数据入库
    dateIntoPostgresql(send, 'volte.v_volte_send_20191113')
    context1 = "派单表数据入库成功"
    write_csv(context1)
    print(context1)
    ##回单数据入库
    dateIntoPostgresql(v_return, 'volte.v_volte_returnvaluation_2019111219')
    context2 = "回单表数据入库成功"
    write_csv(context2)
    print(context2)
    print("任务执行完成")
